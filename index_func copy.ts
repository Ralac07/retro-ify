...
export async function apply_filter(
  ...
  let provided_sizes =
    (resize.width !== undefined ? 1 : 0) +
    (resize.height !== undefined ? 1 : 0);
  ...
  const new_image = image.clone().crop({
    x: 0,
    y: offsetY,
    h: image.height - offsetY * image.height * 2,
    w: image.width - offsetX * image.width * 2,
  });
  ...
  for (let y = 0; y < image.height; y++) {
    ...
    for (let x = 0; x < image.width; x++) {
      ...
      pix = Color(
        (image.getPixelColor(
          x - offsetX * image.width,
          y - offsetY * image.height
        ) -
          255) /
          16 /
          16
...
apply_filter(
  "golden-gate-bridge-1501082087ZNe.jpg",
  "bridge_resize.webp",
  percent(0.5),
  0,
  30,
